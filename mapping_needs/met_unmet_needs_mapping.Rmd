...
First plotting the needs and number of household units.

```{r}
# tgcp_atleast5 <- tgcp_svi[tgcp_svi$ClientZipCode %in% atleast5_zip, ]

tgcp_svi_subset <- unique(cbind(tgcp_svi$ClientZipCode[!is.na(tgcp_svi$ClientZipCode)],
                         tgcp_svi$EP_PCI[!is.na(tgcp_svi$ClientZipCode)], 
                         tgcp_svi$E_HH[!is.na(tgcp_svi$ClientZipCode)]))

zip_code_pci <- data.frame(region = as.character(tgcp_svi_subset[, 1]), value = tgcp_svi_subset[, 2])

tgcp_svi_subset_subset <- tgcp_svi_subset[tgcp_svi_subset[, 1] %in% atleast5_zip, ]

score <- rank(-tgcp_svi_subset_subset[, 2], ties.method = "average", na.last = "keep")
# score <- score / max(score, na.rm = T)

zip_code_rank_pci <- data.frame(region = as.character(tgcp_svi_subset_subset[, 1]), 
                                value = score)



zip_code_hh <- data.frame(region = as.character(tgcp_svi_subset[, 1]), value = tgcp_svi_subset[, 3])
