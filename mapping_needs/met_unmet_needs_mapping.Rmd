...
<!-- Finding the counties with TGCP clients -->

```{r}
zip.regions.tgcp <- zip.regions[zip.regions$region %in% zip_list, ]

tgcp_county_name_code <- unique(cbind(zip.regions.tgcp$county.name, zip.regions.tgcp$county.fips.numeric))

num_clients_county_subset <- left_join(zip_code_num_clients, zip.regions, by = "region")
```

<!-- Finding the counties with >1 clients -->

```{r}
county_summ <- num_clients_county_subset %>% group_by(county.fips.numeric) %>% summarise(county_count = n()) 

county_summ_subset <- county_summ[county_summ$county_count > 8,]
```

```{r}
unique(zip.regions$county.name[zip.regions$county.fips.numeric %in% county_summ_subset$county.fips.numeric])
```

```{r}
county_subset_zip <- zip.regions$region[zip.regions$county.fips.numeric %in% c(county_summ_subset$county.fips.numeric, 37135, 37069)]
```
...
```{r, eval=FALSE}
zip_code_num_clients[(zip_code_num_clients$region %in% north_carolina_zip) & !is.na(zip_code_num_clients$region) & !(zip_code_num_clients$region %in% county_subset_zip),]
```

```{r, eval=FALSE}
zip_code_num_clients[(zip_code_num_clients$region %in% north_carolina_zip) & !is.na(zip_code_num_clients$region) & !(zip_code_num_clients$region %in% zip_code_num_clients$region[zip_code_num_clients$value > 4 &
...
Zooming into the counties that have . 
...
zip_choropleth(zip_code_num_clients,
               # zip_zoom = c(manhattan_les, manhattan_ues),
               county_zoom = c(county_summ_subset$county.fips.numeric),
               title    = "Number of Clients",
               legend   = "Number of Clients", 
               num_colors = 9)
...
zip_choropleth(zip_code_num_clients,
               # zip_zoom = c(manhattan_les, manhattan_ues),
               zip_zoom = zip_code_num_clients$region[zip_code_num_clients$value > 4 &
                                                      !is.na(zip_code_num_clients$region)],
               title    = "Number of Clients",
               legend   = "Number of Clients", 
               num_colors = 9)
