...
---
## Annual Home Income Classification
...
```
```{r,echo=FALSE, fig.show='animate', interval=2} 
# Plot 2_Animation
years <- 2010:2021
par(mar = c(0.01, 0, 0.05, 0.05))
ani.options(convert="C:\\PROGRA~1\\ImageMagick-7.0.11-Q16\\magick.exe",ani.width = 600, ani.height = 600, ani.res = 600)
for (i in years) {
  gcdata <- filter(greenchair2, Year == i)
  fig <- ggplot(gcdata, aes(Homeincome, fill=Homeincome)) +
    geom_bar()+ 
    theme_bw()+
    theme(plot.title = element_text(color="Black", size=14, face="bold", hjust = 0.6),
          plot.caption = element_text(color="Darkgrey", size=7, face="bold",hjust = 0.6),
    ...
    labs(title = "Annual Home Income Classification \n",
         ...
         \nClassification is based on the family size (TotalHHMember) and median income in Raleigh area 
         \nArea median income data source: the HOME Investment Partnerships Program, 2010-2021")+
    geom_text(aes(y = 0, label = paste(Year, " ")), vjust = 0.5, hjust = 0.5)
  print(fig)
}
```
