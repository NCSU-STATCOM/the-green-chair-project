...
# setwd("C:/Users/ywang256/Desktop/Green chairs/")
library(kableExtra)
library(readxl)
library(dplyr)
library(knitr)
...
library(dplyr)
# library(magick)
knitr::opts_chunk$set(echo = FALSE, message = FALSE, fig.align = 'center', fig.width = 5, fig.height = 6)
...
greenchair<- as.data.frame(read_excel("../STATCOM_data.xlsx", na= "NA", col_types = ct))
str(greenchair[,1:21])
...
income_year = greenchair2 %>%
    group_by(Year) %>%
    count(Homeincome) 
    # filter(grepl("Very Low Income", Homeincome)) %>%
    # filter(!grepl("2013", Year))


ggplot(income_year, aes(x=Year, y=n, fill=Homeincome))+
  geom_bar(stat="identity", position="dodge")+labs(x="Year",
  y="Number of Clients", 
  title="Income Percentiles by Year")+
  ...
  theme(plot.title = element_text(color="Black", size=14, face="bold", hjust = 0.6),
          legend.position="bottom")+
  guides(fill=guide_legend(nrow=2, byrow=TRUE))


income_household = greenchair2 %>%
    group_by(TotalHHNumber) %>%
    count(Homeincome) %>%
    filter(!grepl("0|12|13|14", TotalHHNumber)) %>%
    filter(!is.na(TotalHHNumber))


ggplot(income_household, aes(x=TotalHHNumber, y=n, fill=Homeincome))+
  geom_bar(stat="identity", position="dodge")+labs(x="Number of Household Members",
  y="Number of Clients", 
  title="Income Percentiles by Number of Household Members")+
  scale_fill_discrete(name="Median Income Percentile", 
                      labels=c("Above 60%",
                               "Below 60%",
                              "Unknown",
                              "Below 30%")) +
  theme(plot.title = element_text(color="Black", size=12, face="bold", hjust = 0.6),
          legend.position="bottom") +
  guides(fill=guide_legend(nrow=2, byrow=TRUE))

# ggplot(greenchair2, aes(Homeincome, fill=Homeincome)) + geom_bar()+ 
#   facet_grid(Homeincome~Year,scales='free')+
#   theme(plot.title = element_text(color="Black", size=14, face="bold", hjust = 0.6),
#           plot.caption = element_text(color="Darkgrey", size=7, face="bold",hjust = 0.6),
...
#           legend.position="bottom")+
#     scale_fill_manual("",
#                       values = c("Very Low Income" = "#999999", "Low Income" = "#E69F00", 
#                                  "Median" = "#56B4E9","Unknown"= "#009E73"),
#                       labels = c("Very Low Income:\nBelow 30% of Area Median Income",
#                                  "Low Income:\nBetween 30%-60% of Area Median Income",
#                                  "Median:\nAbove 60% of Area Median Income", "Unknown"))+
#     guides(fill=guide_legend(ncol=2))+
#     labs(colour = "Homeincome")+ 
#     xlab("\n Home Income Status") +
#     ylab("Count")+ 
#     labs(title = "Annual Home Income Classification \n",
#          caption = "
#          \nClassification is based on the family size (TotalHHMember) and median income in Raleigh area 
#          \nArea median income data source: the HOME Investment Partnerships Program, 2010-2021")
